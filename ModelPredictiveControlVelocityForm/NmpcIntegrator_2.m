...
% MPC example using Casadi and the single shooting method
...
p.A    = [0 1; -p.wn^2 -2*p.zeta*p.wn];
p.B    = [0; p.wn^2];
p.Bd   = [0; 1];
p.C    = [1 0];
ops.nx = size(p.A,1);       % #state
ops.ny = size(p.C,1);       % #measurements
ops.nu = size(p.B,2);       % #control signals

% simulation parameters
ops.N   = 65;                  % #samples in simulation
...
r          = [1.0*ones(1,floor((ops.N+ops.Np)/2)) ...
                1.0*ones(1,ceil((ops.N+ops.Np)/2))];  % reference signal
d          = 0*[zeros(1,floor((ops.N+ops.Np)/3)) ...
                -0*.7*ones(1,ceil(2*(ops.N+ops.Np)/3))];                  % disturbance
...
u          = zeros(ops.nu,ops.N+1);
u(:,1)     = ops.u0(:,1);

xhorizon   = zeros(ops.nx+1,ops.Np+1,ops.N); % state predictions in the horizon
...
for kk=1:ops.N
    ...
    Deltad = diff(d(:,kk:kk+ops.Np-1),[],2);
    Deltad = 0*[Deltad Deltad(end)];
    %Deltar = diff(r(:,kk:kk+ops.Np-1),[],2);
    %Deltar = [Deltar Deltad(end)];
    Deltar = r(:,kk:kk+ops.Np-1);
    Deltax = diff(x,[],2);
    Deltax = [Deltax(:,1);p.C*x(:,kk)];
    % solve the optimization problem
    [uopt(:,:,kk+1),res.output{kk}] = Nmpc(Deltax,Deltad,Deltar,p,ops);
        
    u(:,kk+1)    = u(:,kk) + uopt(:,1,kk+1);
    
    ops.lab_g0   = res.output{kk}.lam_g;        % this for hotstart
    ops.lab_x0   = res.output{kk}.lam_x;        % this for hotstart
    ops.u0       = uopt(:,:,kk+1);              % this for hotstart
    
    % state evolutions in the prediction horizon
    xhorizon(:,:,kk) = full(ops.xs(Deltax,uopt(:,:,kk+1),Deltad));
    yhorizon(:,:,kk) = full(ops.ys(Deltax,uopt(:,:,kk+1),Deltad));
...
x0  = casadi.SX.sym('x0',ops.nx+1,1);       % initial condition
...
xs  = casadi.SX.sym('xs',ops.nx+1,ops.Np+1);% state
...
% xas = [deltaxs(k);p.C*xs(k)], with deltaxs(k) = xs(k)-xs(k-1)
% note that this system has the inputs deltaus and deltads
% xas       = casadi.SX.sym('xas',ops.nx+ops.ny,ops.Np+1);    % augmented state
% xas(:,1)  = [x0;p.C*x0];                                    % not completely correct
% yas       = casadi.SX.sym('yas',ops.ny,ops.Np);           % augmented output
% for ll=1:ops.Np
%     xas(:,ll+1)  = [xs(:,ll+1)-xs(:,ll);p.C*xs(:,ll+1)]; 
%     yas(:,ll)    = [0 0 1]*xas(:,ll);
% end
...
% end
for ll=1:ops.nu
    ops.lbu = [ops.lbu;-ops.du_max(ll)*ones(ops.Np,1)];
    ops.ubu = [ops.ubu;ops.du_max(ll)*ones(ops.Np,1)];
end
...
%ki = [p.A*x + p.B*u + p.Bd*d;
%ki = [p.A zeros(2,1); p.C*p.A 0]*x + [p.B;p.C*p.B]*u + [p.Bd;p.C*p.Bd]*d;

p.A = [0 1;-p.wn^2 -2*p.zeta*p.wn*x(2)]; 
ki  = [p.A zeros(2,1); p.C*p.A 0]*x + [p.B;p.C*p.B]*u + [p.Bd;p.C*p.Bd]*d;
